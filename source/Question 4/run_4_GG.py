...
import numpy as np
from homeworkLib import MultiClassSVMClassifier
import pandas as pd

if __name__ == "__main__":
    # The hyper params where chosen by using...: hyper_params_tunning.py
    kernel_hyper_param = 0.1; C = 1
    kernel = 'RBF' # 'polynomial' or 'RBF'
    routine = 'cvxopt' # 'quadprog' or 'cvxopt'

    SVM = MultiClassSVMClassifier(kernel_h_p = kernel_hyper_param, C = C, kernel = kernel)

    # In the training set, each example consists of 256 input values
    # and 1 output value representing the coresponding digit
    data_set_train_1 = pd.read_csv("../../Data/Train_1.csv")
    data_set_train_2 = pd.read_csv("../../Data/Train_2.csv")
    data_set_train_8 = pd.read_csv("../../Data/Train_8.csv")
    data_set_train_full = data_set_train_1.append(data_set_train_2.append(data_set_train_8, ignore_index=True), ignore_index=True)
    #data_set_train_full[data_set_train_full[['0']] != 1] = -1
    ...
    data_set_test_1 = pd.read_csv("../../Data/Test_1.csv")
    data_set_test_2 = pd.read_csv("../../Data/Test_2.csv")
    data_set_test_8 = pd.read_csv("../../Data/Test_8.csv")
    data_set_test_full = data_set_test_1.append(data_set_test_2.append(data_set_test_8, ignore_index=True), ignore_index=True)
    #data_set_test_full[data_set_test_full[['0']] != 1] = -1
